...
VOTE_DEAD = -2 #死者,不能投也不能被投
    ...
    def self.vote(uid, v)
        ...
        a = @@r.get(Constance::KEY_VOTE+gid)
        if (a.to_i()>0)
            ...
            self.votework(gid,v,result,usr[3].to_i() - 1).to_s + self.getVoteResult(gid)
            else
...
p result
        ...
        return "此人不能被投,手下留情." if result[v].to_i<VOTE_INIT_S
#p Constance::KEY_L_G_VT+gid, u, i
        result[u] = v
...
#@@r.set
    ...
    def self.getVoteResult(gid,result)
       if ( a = @@r.get(Constance::KEY_VOTE+gid).to_i>0)
           "\n还有#{a}人未投票"
           else
           #(result.length-1)downto(0){|x|}
           "\n完成"
        end
    end
    ...
    def self.createGroup(str, ownerid, human, spy)
        ...
        initGroup(gid, ownerid,human,spy,s , str.length==0).to_s+"游戏编号:#{gid}\n其他人可输入此编号，开始游戏。"
    end
	#创建redis中组相应的数据
    #isAutoWords
